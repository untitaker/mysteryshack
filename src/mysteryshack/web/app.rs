...
use config;
use super::utils::{preconditions_ok, EtagMatcher, SecurityHeaderMiddleware, XForwardedMiddleware,
                   FormDataHelper, get_account_id};
use super::oauth;
...
pub fn run_server(config: config::Config) {
    ...
    if config.use_proxy_headers {
        chain.link_before(XForwardedMiddleware);
    }
    ...
    mount.mount("/", chain);
    mount.mount("/static/",
                staticfile::Static::new(Path::new("./src/static/")));
...
fn user_node_response(req: &mut Request) -> IronResult<Response> {
    ...
    let (userid, path_str) = {
        ...
        let path_str =
            String::from_utf8(url::percent_encoding::percent_decode(parts.find("path")
                                                                         .unwrap_or("")
                                                                         .as_bytes()))
                .unwrap();
        (userid, path_str)
    ...
    let permissions = user.get_permissions(access_token.as_ref().map(Deref::deref), &path_str[..]);
...
fn user_login_get(request: &mut Request) -> IronResult<Response> {
    ...
    r.set_mut(Template::new("login", {
        ...
        request.get_ref::<urlencoded::UrlEncodedQuery>()
               .ok()
...
fn user_login_post(request: &mut Request) -> IronResult<Response> {
    ...
    let url = request.get_ref::<urlencoded::UrlEncodedQuery>()
                     .ok()
    ...
    if &url.scheme != &request.url.scheme || &url.host != &request.url.host ||
...
fn user_dashboard(request: &mut Request) -> IronResult<Response> {
    ...
    match request.method {
        Method::Get => Ok(Response::with(status::Ok).set(Template::new("dashboard", {
            let mut rv = collections::BTreeMap::new();
            rv.insert("account_id".to_owned(),
                      get_account_id(&user, &request).to_json());
        ...
        Method::Post => {
            ...
            let (action, token) = some_or!(request.get_ref::<urlencoded::UrlEncodedBody>()
                                                  .ok()
                                                  .map(|query| {
                                                      (query.get_only("action").clone(),
                                                       query.get_only("token").clone())
                                                  }),
                                           status::BadRequest);
...
fn oauth_entry(request: &mut Request) -> IronResult<Response> {
    ...
    let oauth_request = match oauth::OauthRequest::from_http(request) {
        Ok(x) => x,
        Err(e) => return Ok(e.get_response().unwrap_or_else(|| {
            Response::with(status::BadRequest)
                .set(Template::new("oauth_error",
                                   json::Json::Object({
                                       let mut rv = collections::BTreeMap::new();
    ...
    match request.method {
        Method::Get => Ok(Response::with(status::Ok)
                              .set(Template::new("oauth_entry", oauth_request.to_json()))),
        Method::Post => {
            check_csrf!(request);
            let allow = some_or!({
                                     let formdata =
                                         itry!(request.get_ref::<urlencoded::UrlEncodedBody>());
    ...
    fn respond_put(self, request: &mut Request) -> IronResult<Response> {
...

        {
            let content_type = match request.headers.get_raw("content-type") {
                ...
                Some(_) => return Ok(Response::with((status::BadRequest,
                                                     "ONE content type header required."))),
