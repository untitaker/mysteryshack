...
pub fn main() {
    let matches = App::new("mysteryshack")
                      .version("0.1.0")
                      .author("Markus Unterwaditzer & contributors")
                      .about("A remoteStorage server.")
                      .args_from_usage("-c --config=[CONFIG] 'Use specified config file, \
                                        defaults to ./config'")
                      .subcommand(SubCommand::with_name("serve").about("Start server"))
    ...
    if let Some(_) = matches.subcommand_matches("serve") {
        ...
        if let Some(user_create_matches) = user_matches.subcommand_matches("create") {
            ...
            let password_hash =
                match models::PasswordHash::from_password(utils::double_prompt("Password for \
                                                                                new user: ")) {
                    Ok(x) => x,
                    Err(e) => {
            ...
            match models::User::create(&config.data_path, username)
                      .map(|user| user.set_password_hash(password_hash)) {
                Ok(_) => (),
                Err(e) => {
            ...
            println!("You are about to delete the user {} and ALL the user's user data. This
                     ...
                     username);
            if !utils::prompt_confirm(format!("Do you want to delete the user {}?", username),
                                      false) {
                println!("Aborted!");
                process::exit(1);
            }
